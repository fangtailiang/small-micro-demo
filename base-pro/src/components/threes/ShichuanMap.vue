<template>
    <div class="sichuan-wrap" ref="refMap"></div>
</template>

<script setup>
    import * as THREE from "three";
    import { onMounted, ref } from "vue";
    import { getMap } from "@/api/threes/map";
    ...
    const mountedMap = () => {
        const width = refMap.value.offsetWidth;
        const height = refMap.value.offsetHeight;
        ...
        refMap.value.appendChild(labelRenderer.domElement);

        const renderer = new THREE.WebGLRenderer({ alpha: true });

        renderer.setSize(width, height);
        refMap.value.appendChild(renderer.domElement);
